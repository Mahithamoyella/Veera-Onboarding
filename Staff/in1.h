...
<body>
    ...
    <script>
            ...
            employees.forEach(employee => {
                ...
                card.innerHTML = `
                    <div class="card-header">
                        <img class="profile-pic" src="http://54.209.116.102:3075/api/onboarding/${employee.id}/file/profilePic" alt="Profile Picture" onerror="this.src='https://via.placeholder.com/50'; this.alt='No Profile Picture'">
                        <div class="info-container">
        ...
        async function showEmployeeDetails(employeeId) {
            try {
                const response = await fetch(`http://54.209.116.102:3075/api/onboarding/${employeeId}`);
                if (!response.ok) {
                    throw new Error(`HTTP ${response.status}: ${await response.text()}`);
                }
                ...
                document.getElementById('modalProfilePic').src = `http://54.209.116.102:3075/api/onboarding/${employee.id}/file/profilePic`;
