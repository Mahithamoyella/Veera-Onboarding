...
<body>
    ...
    <script>
        ...
        async function fetchActiveEmployees() {
            try {
                const response = await fetch('http://3.86.112.83:3075/api/employees/active');
                if (!response.ok) {
            ...
            employees.forEach(employee => {
                ...
                card.innerHTML = `
                    <div class="card-header">
                        <img class="profile-pic" src="http://3.86.112.83:3075/api/onboarding/${employee.id}/file/profilePic" alt="Profile Picture" onerror="this.src='https://via.placeholder.com/50'; this.alt='No Profile Picture'">
                        <div class="info-container">
        ...
        async function showEmployeeDetails(employeeId) {
            try {
                const response = await fetch(`http://3.86.112.83:3075/api/onboarding/${employeeId}`);
                if (!response.ok) {
                    throw new Error(`HTTP ${response.status}: ${await response.text()}`);
                }
                ...
                document.getElementById('modalProfilePic').src = `http://3.86.112.83:3075/api/onboarding/${employee.id}/file/profilePic`;
            ...
            document.getElementById('searchInput').addEventListener('input', async (e) => {
                ...
                try {
                    ...
                    const filteredEmployees = employees.filter(employee => {
                        const searchString = `${employee.full_name} ${employee.department || ''} ${employee.emp_id || ''}`.toLowerCase();
