-- Create the database (only works outside a DB or from superuser)
-- If you're running this from within an existing DB context, skip this.
-- CREATE DATABASE employee_db;

-- Connect to the database (only works in interactive psql, not in .sql scripts)
-- \c employee_db;

-- Instead, in Docker, set POSTGRES_DB=employee_db in docker-compose
...
CREATE TABLE IF NOT EXISTS onboarding_records (
  ...
  ssc_year INTEGER NOT NULL CHECK (ssc_year BETWEEN 1900 AND 2026),
  inter_institution VARCHAR(200) NOT NULL,
  inter_year INTEGER NOT NULL CHECK (inter_year BETWEEN 1900 AND 2026),
  degree VARCHAR(100) NOT NULL,
  institution VARCHAR(200) NOT NULL,
  graduation_year INTEGER NOT NULL CHECK (graduation_year BETWEEN 1900 AND 2026),
